...
export const BANGKOK_LAYERS: LayerSpec[] = [
  ...
  { id: 'longdo-traffic',  label: 'TRAFFIC · iTIC LIVE', description: 'Measured road-segment congestion from Longdo\'s live traffic tiles — the same rendering behind iTIC Live (live.iticfoundation.org). Every road painted green/amber/red; tiles re-keyed every 5 minutes so each visit sees current state. (Earlier "SDK-only" note was wrong — plain XYZ tiles verified working.)', source: 'Longdo', defaultOn: false, status: 'live', category: 'transit' },
  { id: 'itic-incidents',  label: 'INCIDENTS · iTIC LIVE', description: 'Live accidents, breakdowns, road closures and diversions from the iTIC / Longdo Events feed — Thai + English descriptions, click any marker for detail. The same incident rail iTIC Live shows, refreshed every visit.', source: 'Longdo', defaultOn: false, status: 'live', category: 'transit' },
